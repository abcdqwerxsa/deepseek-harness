...
- Concurrent `withTenant` calls for one tenant share a single spawn (per-tenant in-flight dedup); capacity slots are reserved synchronously so queued wake-ups never overshoot the cap.
...
- Real-spawn coverage lives in `tests/orchestrator.e2e.ts` (two tenants, cap queueing, idle reaping); manager logic alone is covered by fast fake-runtime tests.
